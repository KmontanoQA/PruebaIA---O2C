...
*-- Instanciar la clase del propio PRG y levantar ciclo de eventos
LOCAL oForm
oForm = NEWOBJECT("frmLogin")
oForm.Show()
READ EVENTS
RETURN
...
DEFINE CLASS frmLogin AS Form
    ...
    ADD OBJECT lblLeftDescription AS Label WITH ;
        Caption = "Accede para gestionar ordenes, facturacion y existencias de manera eficiente dentro del ERP.", ;
    ...
    PROCEDURE DoLogin
        LOCAL lcEmail, lcPassword, lcPayload, loHTTP, lcResponse, lcMessage
        ...
        DO CASE
            CASE loHTTP.Status = 200
                ...
                IF EMPTY(gcToken)
                    THIS.ShowStatus("La respuesta no contiene un token valido.", .T.)
                    RETURN .F.
                ENDIF
                ...
                THIS.Release()
                *-- Abre tu formulario principal si existe como .SCX:
    ...
    *-- Extrae un string JSON: {"key":"value"}
    PROCEDURE JsonGetString
        LPARAMETERS lcJSON, lcKey
        LOCAL lcNeedle
        lcNeedle = '"' + lcKey + '":"'
        RETURN STREXTRACT(lcJSON, lcNeedle, '"', 1, 1)
    ENDPROC
